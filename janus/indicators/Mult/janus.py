...
class JANUS:
    ...
    def get_current_leader_dom_vs_cma(self) -> Tuple[np.ndarray, np.ndarray, float]:
        """
        Get current DOM values for leaders and compare against latest CMA
        ...
        Tuple[np.ndarray, np.ndarray, float]:
            ...
            - Current CMA value
        """
        # Get latest CMA value (using max lookback)
        idx = self.max_CMA - self.min_CMA
        current_cma = self.CMA_smoothed[idx]
        
        # Find current leaders using latest RM values
        k = max(0, int(self.spread_tail * (self.n_markets + 1)) - 1)
        n = k + 1  # Number of leader markets
        
        # Get latest RM values and sort
        latest_rm = self.rm[-1]  # Gets last row
        order = np.argsort(latest_rm)
        
        # Get leader indices (top n markets by RM)
        leader_indices = order[-n:]
        
        # Get their current DOM values
        leader_doms = self.dom[-1, leader_indices]  # Gets last row for these indices
        
        return leader_indices, leader_doms, current_cma
    
    def get_all_markets_dom_vs_cma(self) -> Tuple[np.ndarray, np.ndarray, float]:
        """
        Get current DOM values for all markets and compare against latest CMA.
        Markets are sorted by their current RM values in descending order.
        
        Returns:
        --------
        Tuple[np.ndarray, np.ndarray, float]:
            - Market indices (sorted by RM)
            - Their DOM values 
            - Current CMA value
        """
        # Get latest CMA value (using max lookback)
        ...
        # Get latest RM values and sort all markets
        latest_rm = self.rm[-1]  # Gets last row
        market_indices = np.argsort(latest_rm)[::-1]  # Sort in descending order
        
        # Get current DOM values for all markets
        market_doms = self.dom[-1, market_indices]  # Gets last row for these indices
        
        return market_indices, market_doms, current_cma