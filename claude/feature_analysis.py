...
class FeatureAnalysis:
    def __init__(self, X: pd.DataFrame, y: pd.Series, model: BaggingClassifier, cv_folds: CombinatorialPurgedKFold, combined_weights: pd.Series):
        self.X = X
        self.y = y
        ...
        #     print(self.ortho_features)
            
        #     # Check for NaN/inf in result
        #     if np.any(np.isnan(self.ortho_features)) or np.any(np.isinf(self.ortho_features)):
        #         print("Warning: NaN/inf found after initial orthogonalization. Trying with adjusted parameters...")
                
        #         # Try again with more conservative parameters
        #         X_cleaned = X.copy()
        #         # Clip extreme values
        #         for col in X_cleaned.columns:
        #             q1, q99 = X_cleaned[col].quantile([0.01, 0.99])
        #             X_cleaned[col] = X_cleaned[col].clip(q1, q99)
                
        #         # Standardize the data
        #         X_cleaned = (X_cleaned - X_cleaned.mean()) / X_cleaned.std()
                
        #         # Try with lower variance threshold
        #         self.ortho_features = get_orthogonal_features(X_cleaned, variance_thresh=0.90)
        
        # except Exception as e:
        #     print(f"Error during orthogonalization: {str(e)}")
        #     print("Falling back to original features...")
        #     self.ortho_features = X.values
        ...
        fs = FeatureStorage("ortho.parquet")
    ...
    def analyze_feature_importance(self):
        ...
        plot_feature_importance(
            ...
            save_fig=True,
            output_path=f'./images/Ortho_MDI_feature_importance.png'
        )
        ...
        plot_feature_importance(
            ...
            oos_score=self.oos_score,
            save_fig=True,
            output_path=f'./images/Ortho_MDA_feature_importance.png'
        )
        ...
        plot_feature_importance(
            ...
            oos_score=self.oos_score,
            save_fig=True,
            output_path=f'./images/Ortho_SFI_feature_importance.png'
        )
        ...
        plt.savefig('correlation_matrix.png')  # PNG format
        plt.close()
        ...
        plot_feature_importance(
            ...
            oos_score=self.oos_score,
            save_fig=True,
            output_path=f'./images/Ortho_CMDI_feature_importance.png'
        )
        ...
        plot_feature_importance(
            ...
            oos_score=self.oos_score,
            save_fig=True,
            output_path=f'./images/Ortho_CMDA_feature_importance.png'
        )
